...
class CMB2_Field {
	...
	public function sanitization_cb( $meta_value ) {
		...
		$cb = $this->mayCC_callback( 'sanitization_cb' );
		if ( false === $cb ) {
	 ...
	 */
	public function mayCC_callback( $cb ) {
	...
	public function escaped_value( $func = 'esc_attr', $meta_value = '' ) {
		...
		if ( $cb = $this->mayCC_callback( 'escape_cb' ) ) {
			// Ok, callback is good, let's run it.
			return call_user_func( $cb, $meta_value, $this->args(), $this );
		}
	...
	public function render_field() {
		...
		if ( $cb = $this->mayCC_callback( 'render_row_cb' ) ) {
			// Ok, callback is good, let's run it.
	...
	public function get_param_callback_result( $param, $echo = true ) {
		...
		if ( $cb = $this->mayCC_callback( $param ) ) {
			if ( $echo ) {
	 ...
	 */
	public function mayCC_set_attributes( $attrs = array() ) {
